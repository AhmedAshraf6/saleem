...
import { changeStepNumber, setIhgznyProduct } from '../features/user/userSlice';
export default function IhgznyBook() {
  const dispatch = useDispatch();
  const { isPending: isLoadingFetchData, data: products } = useQuery({
    queryKey: ['allBookingProductsFromIhgzny'],
    queryFn: async () => {
      const { data } = await customFetch(`/get-types`);
      return data.data;
    },
  });
    ...
    <>
      ...
      <div className='allServices flex flex-col gap-2 sm:gap-4 h-[75vh] overflow-y-auto'>
        ...
        <div className='rounded-xl border-[1px] border-[#E1E2E2] bg-[#FBFBFE] py-2 px-3 sm:py-3 sm:px-4'>
          <h2 className='text-lg font-bold my-1 sm:my-3'>
            اختر خدمتك في احجزني
          </h2>
                    ...
                    <div className=' flex justify-between flex-wrap gap-3 p-3'>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          dispatch(setIhgznyProduct(product));
                        }}
                      >
                        <img
                          src={
      ...
      <div className='absolute bottom-[20px] left-[50%] -translate-x-[50%]'>
        <button
          ...
          onClick={() => dispatch(changeStepNumber(3))}
